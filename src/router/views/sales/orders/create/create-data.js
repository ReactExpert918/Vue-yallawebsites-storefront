const shippingData = [
  {
    id: 1,
    shippingMethod: 'UPS',
    shippingCost: '5.00',
  },
  {
    id: 2,
    shippingMethod: 'Fedex',
    shippingCost: '7.00',
  }
];
const viewData = [
  {
      id: 1,
      orderStatus: 'Processing',
      orderNumber: '231234',
      orderDate: '02/13/20',
      customerName: 'Michael Figg',
      customerEmail: 'michael@nvera.com',
      invoiceNumber: '2FDAS31J',
      invoiceDate: '02/10/20',
      creditMemoNumber: '412312',
      creditMemoDate: '12/10/20',
      shipmentNumber: '92318031',
      shipmentDate: '12/10/20',
      billingAddressCustomerName: 'John Doe',
      billingAddressStreetAddress: '14 street address 1',
      billingAddressCity: 'London',
      billingAddressPostCodeZip: 'W1J7BU',
      billingAddressCountry: 'England',
      billingAddressTelephoneNumber: '02138318922',
      shippingAddressCustomerName: 'John Doe',
      shippingAddressStreetAddress: '14 street address 1',
      shippingAddressCity: 'London',
      shippingAddressPostCodeZip: 'W1J7BU',
      shippingAddressCountry: 'England',
      shippingAddressTelephoneNumber: '02138318922',
      paymentMethod: 'Credit / Debit',
      paymentCurrency: 'GBP',
      orderCurrencySymbol: '£',
      paymentAmount: '120.31',
      shippingMethod: 'UPS',
      shippingCost: '5.00',
      subtotal: '100.31',
      total: '123.31'
    }
];
const paymentData = [
  {
    id: 1,
    paymentMethod: 'Credit / Debit',

  },
  {
    id: 2,
    paymentMethod: 'Stripe',

  }
];
const allProductsData = [
  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }, {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    },  {
      selected: false,
      productName: 'Product',
      productSku: 'ASDS-213',
      productPrice: 23.12,
      productQty: 31,
      status: 'Enabled',
    }
];
const customerData = [
  {
    selected: false,
    customerId: 12,
    customerName: 'Customer Name',
    customerEmail: 'customer@gmail.com',
    customerBillingFirstName:'John',
    customerBillingLastName: 'Doe',
    customerBillingAddressLine1: '14 street address 1',
    customerBillingCity: 'London',
    customerBillingPostZip: 'W1J7BU',
    customerBillingCounty:'Essex',
    customerBillingCountry:'England',
    customerBillingPhoneNumber: '02138318922',
    customerShippingFirstName:'John',
    customerShippingLastName: 'Doe',
    customerShippingAddressLine1: '14 street address 1',
    customerShippingCity: 'London',
    customerShippingPostZip: 'W1J7BU',
    customerShippingCounty:'Essex',
    customerShippingCountry:'England',
    customerShippingPhoneNumber: '02138318922'
  },
  {
    selected: false,
    customerId: 13,
    customerName: 'Customer Name',
    customerEmail: 'customer@gmail.com',
    customerBillingFirstName:'John',
    customerBillingLastName: 'Doe',
    customerBillingAddressLine1: '14 street address 1',
    customerBillingCity: 'London',
    customerBillingPostZip: 'W1J7BU',
    customerBillingCounty:'Essex',
    customerBillingCountry:'England',
    customerBillingPhoneNumber: '02138318922',
    customerShippingFirstName:'John',
    customerShippingLastName: 'Doe',
    customerShippingAddressLine1: '14 street address 1',
    customerShippingCity: 'London',
    customerShippingPostZip: 'W1J7BU',
    customerShippingCounty:'Essex',
    customerShippingCountry:'England',
    customerShippingPhoneNumber: '02138318922'
  },
  {
    selected: false,
    customerId: 14,
    customerName: 'Customer Name',
    customerEmail: 'customer@gmail.com',
    customerBillingFirstName:'John',
    customerBillingLastName: 'Doe',
    customerBillingAddressLine1: '14 street address 1',
    customerBillingCity: 'London',
    customerBillingPostZip: 'W1J7BU',
    customerBillingCounty:'Essex',
    customerBillingCountry:'England',
    customerBillingPhoneNumber: '02138318922',
    customerShippingFirstName:'John',
    customerShippingLastName: 'Doe',
    customerShippingAddressLine1: '14 street address 1',
    customerShippingCity: 'London',
    customerShippingPostZip: 'W1J7BU',
    customerShippingCounty:'Essex',
    customerShippingCountry:'England',
    customerShippingPhoneNumber: '02138318922'
  }
];


export { paymentData, shippingData, viewData, allProductsData, customerData };
